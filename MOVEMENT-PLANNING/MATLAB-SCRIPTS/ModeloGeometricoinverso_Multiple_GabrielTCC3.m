...
dream.y =     [  80,  80,  80,  80,  80,  80,  80,  80,  80,  80,  80,  80,  80];
dream.z =     [ 105,  75,  45,  45,  45,  45,  45,  15,  45,  75,  75,  75, 105];
dream.psi =   [ -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90];
dream.theta = [-180,-180,-180,-180,-180,-180,-180,-180,-180,-180,-180,-180,-180];
...
granularity = 10; % fixo em N passos 5//50//500
step = 1;
objIte = 1;
%% Parâmetros de otimização

%parametros das iteracoes
resolution = 1;                                %delta rotacao base incial

i = 0;                                          %numero de iteracoes

min_error = 0.1;                                %erro desejado
...
while objIte <= length(dream.x) % while objIte <= 2 %while objIte <= length(dream.x)
    while step <= granularity
        ...
        solution.q((objIte-1)*granularity+step,1:6) = now.q;
        solution.terminal((objIte-1)*granularity+step,1) = now.x;
        solution.terminal((objIte-1)*granularity+step,2) = now.y;
        solution.terminal((objIte-1)*granularity+step,3) = now.z;
        solution.terminal((objIte-1)*granularity+step,4) = now.psi;
        solution.terminal((objIte-1)*granularity+step,5) = now.theta;
        solution.terminal((objIte-1)*granularity+step,6) = now.phi;
        step = step + 1;
    end
