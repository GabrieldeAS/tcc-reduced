function robotic_arm_print2(original,dream, q, name )
...
%% adição da reta da trajetória
%start_line = [original.x,original.y,original.z];
%end_line = [dream.x,dream.y,dream.z];
t = 0:0.01:1;
X = original.x + t*(dream.x-original.x);
Y = original.y + t*(dream.x-original.y);
Z = original.z + t*(dream.x-original.z);
